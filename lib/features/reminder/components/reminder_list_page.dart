...
//   权限提示 通知权限 / 「闹钟和提醒」未开启时的内联提示条（不强行打断，可点去设置）
...
class _ReminderListPageState extends ConsumerState<ReminderListPage> {
  ...
  /// 权限状态（null = 还没查完，不显示提示条）
  ({bool notifications, bool exactAlarm})? _caps;
  ...
  Widget _body(BuildContext context, List<ReminderItem> items) {
    ...
    return CustomScrollView(
      slivers: [
        ...
        SliverToBoxAdapter(child: _permissionHint(context)),
  ...
  /// 权限提示条：通知权限 / 「闹钟和提醒」未开启时内联展示（可点「去设置」，不打断浏览）
  Widget _permissionHint(BuildContext context) {
    final caps = _caps;
    if (caps == null || (caps.notifications && caps.exactAlarm)) {
      return const SizedBox.shrink();
    }
    final t = context.theme;
    final messages = <String>[
      if (!caps.notifications) '通知权限未开启，到点无法弹通知',
      if (!caps.exactAlarm) '「闹钟和提醒」未开启，闹钟可能不准时',
    ];
    return Padding(
...
class _ReminderEditorState extends ConsumerState<_ReminderEditor> {
  ...
  Future<void> _ensureAlarmPermission() async {
    if (await NotificationService.exactAlarmAllowed) return;
    final ok = await NotificationService.requestExactAlarmPermission();
    if (!ok && mounted) {
      showFToast(
        context: context,
        title: const Text('未授予「闹钟和提醒」权限'),
        description: const Text('闹钟可能不准时，可到系统设置开启'),
      );
    }
  }
