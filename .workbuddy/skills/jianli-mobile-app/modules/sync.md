...
## 局域网同步（协议 v1，双端已全通）
- **文案口径（2026-09-13 用户定，改动前先看这条）**：本工程面向用户的文案统一称「**多端数据拉取/发送同步**」，**UI / 文档 / 注释里都不要提 LocalSend 或任何第三方产品名**（工具页入口卡副标题、同步页横幅、关于页「同步协议」行、`sync_discovery.dart` 注释、`core/sync/README.md` 与本文档均已于 2026-09-13 全量清理）。协议出处信息已不再对外表述。
