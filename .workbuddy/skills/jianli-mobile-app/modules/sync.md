...
## 局域网同步（协议 v1，双端已全通）
- **文案口径（2026-09-13 用户定，改动前先看这条）**：本工程面向用户的文案统一称「**多端数据拉取/发送同步**」，**UI / 文档 / 注释里都不要提 LocalSend 或任何第三方产品名**（工具页入口卡副标题、同步页横幅、关于页「同步协议」行、`sync_discovery.dart` 注释、`core/sync/README.md` 与本文档均已于 2026-09-13 全量清理）。协议出处信息已不再对外表述。**2026-09-22 补充**：同步能力已并入「备份与恢复」页（`/backup`，见 `features.md` 的 `backup` 行），工具页条目/设置面板 tile 的对外表述改为「**局域网同步 · 数据库导入导出**」，页内横幅改由父页传入（`bannerTitle: '备份与恢复'`）；「不提第三方产品名」这条不变。
...
- `features/todo/components/todo_sheets.dart`（**新增，全部 `showFSheet`+`SheetSurface`**）：`showTodoEditSheet`(标题/描述/优先级/到期/状态/截止提醒(次+间隔+单位)/重复(日|周+间隔+星期+结束)/父任务多选/标签多选+新建/记录进展入口)、`showTodoDetailSheet`(**只读详情**：标题/描述/状态/优先级/重复标记 chips + 到期时间/截止提醒/重复/关联父任务(可下钻)/标签/子任务进度/创建时间/完成时间，底部「记录进展 + 编辑」，返回 `TodoDetailResult(action, item)`；**单击待办的唯一入口 `openTodoDetail` 就建立在它之上**，三处调用点见下)、`showTodoFilterSheet`(优先级/状态/标签/显示已完成/显示模板/分组)、`showTodoTagSheet`(标签多选+新建)、`showTodoParentSheet`(父任务多选)、`showTodoDateTimeSheet`(**自绘月历+时分步进器**，dateOnly 用于重复结束日)、`showRecordProgressSheet`(写主题对话)、`showTodoActionSheet`(编辑/记录进展/删除)、`showTodoConfirmSheet`(危险确认，取消 false/确认 true)。
  **单击 = 查看（2026-09-12）**：`openTodoDetail(context, ref, item, allTodos:, tags:)` 是列表卡片(`todo_tile` 的 `onTap`)/卡片视图(`todo_card_view`)/日历当天抽屉(`showTodoDaySheet`)三处**共用**的单击入口——先开只读详情，再按出口动作转编辑表单（`showTodoEditSheet` → `upsertTodo`）或记录进展。**新增第 4 条调用点时复用它，别各自写一遍**（否则会出现某处点开是查看、某处是编辑）。日期文案共用 `todo_filter.dart` 的 `formatTodoDue`（`todo_tile` 里原先私有的 `_fmtDue` 已删）。
