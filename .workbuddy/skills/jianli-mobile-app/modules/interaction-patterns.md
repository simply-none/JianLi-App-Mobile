...
- 🔴 **【键盘弹起时底部条必须抬起，否则「按钮点了没反应」】（2026-09-18 习惯编辑页实踩；2026-09-19 全 App 收口）**：lg 抽屉按 §一 走 `sheetMaxHeightFull`（固定 80vh、**不扣键盘**）+ 调用点 `resizeToAvoidBottomInset: false` ⇒ 软键盘是从屏幕底部**覆盖**抽屉的。按钮死贴抽屉底部时会被键盘**完全遮住**，用户在输入框打完字直接点保存，实际点到的是键盘区域 —— 观感就是「保存按钮没效果、弹窗也不关」，且**只在键盘弹起时复现**（不动输入框就正常），极难自查。**修法：给 bottomBar 包一层 `Padding(bottom: MediaQuery.of(context).viewInsets.bottom)`**（只抬按钮，抽屉高度与滚动区不变，不违反「lg 不扣键盘」红线）。`_habitSheetPanel` 已内置；**2026-09-19 起共享 `SheetScaffold`（`sheet_form.dart`）、待办 `_sheetScaffold`（`todo_sheets.dart`）也已内置（sm/md 走 `sheetMaxHeight` 已扣键盘、不重复补偿）**，自绘面板（如 `annotation_sheet._editNote`）仍需照抄。机制实证（forui 0.26 源码）：sheet builder 子树内 `MediaQuery.viewInsets` 完整保留（`FSheet` 只 `removePadding`），骨架内可直接读。
...
- 🔴 **【「关抽屉」只能依赖「写库成功」—— 别把 `pop` 排在视觉副作用之后】（2026-09-23 电子书「编辑标注」实报「点 2 次保存才关弹窗」）**：上一条禁止的是「写库没 await 就 pop」，**不是**「pop 前面不能有任何 await」。反过来的坑同样致命：把 `await onSaved(...)`（WebView 覆盖层重绘 / 刷新动画 / 任何走 JS 桥或第三方 SDK 的副作用）排在 `Navigator.pop` **之前** —— 那条链一旦慢/抛/挂，pop 就永远到不了；若 `catch` 里只复位 `saving` 而不关窗，用户看到的就是**「第一次点保存：数据已存但抽屉不关；第二次点：命中『无改动』分支才立刻 pop」= 要点两次**。**正解：`await` 写库（失败留在抽屉里 `showFToast` 报错）→ 立刻 `pop` → 视觉副作用放到关窗之后 fire-and-forget（内部再自己 try/catch 吞错）**；视觉副作用失败不该拦住用户离场，下次进页/重排会自动补上。
- 🔴 **【pop 必须收口成单一入口，且只弹一层】**：`Navigator.pop(context)` 不要散落在保存/取消/删除各处，抽一个 `_popSheet()`：`if (_closing || !mounted) return; final nav = Navigator.of(context); if (!nav.canPop()) return; _closing = true; nav.pop();`。**嵌套抽屉（在列表抽屉之上再叠一层编辑抽屉）退出动画期间连点会各 pop 一次，把外层列表抽屉也关掉** —— `_closing` 闩锁是唯一屏障。⚠️ 这与上一条的 `saving` 禁用态要**同时**存在：闩锁防多弹一层，禁用态防重复写库。
