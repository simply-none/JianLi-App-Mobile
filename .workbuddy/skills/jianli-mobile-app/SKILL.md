...
9. 【**弹窗（底部抽屉）高度只有三档 + 键盘扣减**（2026-09-12 重定，全局强制）】完整规格见 `modules/interaction-patterns.md` §1，要点：
   ① **只有三档**：`SheetSize.sm / md / lg` = **30% / 50% / 80%**，**禁止第四种**，新增弹窗必须挂档（不要在调用点写裸比例）；比例值 = `AppTokens.sheetHeightSm/Md/Lg`。
   ② 高度公式 = **（屏幕高 − 键盘高）× 档位**（`sheetMaxHeight()`，`lib/app/ui/sheet_surface.dart`）——**必须减键盘高**：forui `ShiftedSheet` 用 `dy = max(0, H − 抽屉高 − 键盘高)` 摆放抽屉，抽屉高一旦超过「H − 键盘高」`dy` 就被夹到 0 **不再上移**，抽屉会被键盘从底下**盖住**（现象：底部「保存 / 查询」被键盘压住点不到）。
   ③ 抽屉**定高**（`SizedBox(height:)`，不是 maxHeight），内容超出由**中间滚动区**承担，绝不撑高；键盘展开**不改档**，只按可用高度收缩。
   ...
   ⑤ ⚠️ **旧结论「键盘弹起时整张抽屉抬到键盘上方」是错的**（`mainAxisMaxRatio: null` 时期的注释 + 文档），已在本轮修正，别再照抄；`mainAxisMaxRatio` 现传 `AppTokens.sheetHeightLg`(0.8)，只当「禁止 100vh」的保险。
...
| **交互模式** | `modules/interaction-patterns.md` | **弹窗三档制（30/50/80%）+ 标题字号上限 + 键盘扣减**、**列表吸顶以搜索框为锚点**、共有交互模式目录、**待办模块设计总结** | **写/改任何弹窗、确认、筛选抽屉、列表条目点击、列表吸顶时（先读这个）** |
