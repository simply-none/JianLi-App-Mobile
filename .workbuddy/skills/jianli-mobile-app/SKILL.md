...
- **间距随字号联动（仅垂直缝隙）**：`FScaffold.childPadding` 在主题构建时按同一缩放系数（`baseFontSize/forui默认md`）缩放；但**页面左右边距固定 `AppTokens.pagePadding = 12`，不随字号缩放**（见「全局配置」小节）。垂直缝隙 `listTopGap`/`pageBottomGap` 仍走 `*Of(context)` 随字号缩放。
