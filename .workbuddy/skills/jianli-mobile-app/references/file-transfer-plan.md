...
- 文件名安全：收端仅取 basename、过滤非法字符、重名追加 ` (n)`；只允许落 `Documents/渐离App文件互传/`。
...
- **#20 历史分页 + 自动清理**：`TransferRepository.list({limit,offset})` 供分页；`trim(1000)` 在 `/file/end` 后执行，超出删最旧。页面「传输记录」默认取 30 条 + 「加载更多」。
